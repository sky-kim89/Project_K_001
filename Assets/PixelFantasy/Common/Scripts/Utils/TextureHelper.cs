...
namespace Assets.PixelFantasy.Common.Scripts.Utils
{
    ...
    public static class TextureHelper
    {
        ...
        public static Color32[] Repaint(Color32[] pixels, int width, int height, Color32 paint, List<Color32> palette)
        {
            ...
            {
                paint = palette.OrderBy(i => ColorDiff.GetEuclidean(i, paint, true)).First();
            }

            var index = palette.IndexOf(paint) - 1;

            while (index + colors.Count > palette.Count)
            {
                index--;
            }

            var replacement = palette.GetRange(index, colors.Count).OrderBy(i => ((Color) i).grayscale).ToList();
