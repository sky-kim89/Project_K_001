...
public enum AbilityId
{
    ...
    C07 = 307, // 혼령 집결     — OnBattleStart:  이번 런 사망 병사 수 × 5% 공격력·체력 강화 (아군 전체)
    C08 = 308, // 황금 탐욕     — 시스템:         스테이지 골드 보상 +30%
    C09 = 309, // 성장 촉진     — 시스템:         장군 경험치 획득량 +30%
    C10 = 310, // 시간 왜곡     — OnBattleStart:  스킬 쿨다운 감소 +0.35, 공격력 -10%
    ...
    D01 = 401, // 기사 달인     — Knight          주기적으로 타겟에게 300% 피해 돌진 (6초 쿨다운)
    D02 = 402, // 궁수 달인     — Archer          일반 공격 시 50% 확률로 2번째 적에게도 공격
    D03 = 403, // 마법사 달인   — Mage            일반 공격 시 1% 확률로 보유 스킬 즉시 발동
    D04 = 404, // 방패병 달인   — ShieldBearer    넉백 완전 무시 + 피해의 25% 반사
}
